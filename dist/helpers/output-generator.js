...
export class OutputGenerator {
    constructor(options, previousWrittenFiles) {
        /**
     ...
     */
    forPath(nestedDirPath) {
        const generator = new OutputGenerator(this.options, this.previousWrittenFiles);
        generator.rootGenerator = this.rootGenerator;
        generator.boundNestedPath = [...this.boundNestedPath, nestedDirPath];
        return generator;
    }
    async markAsWritten(filePath) {
     ...
     */
    outputPathTo(...names) {
        return path.join(this.options.outputDirectory, ...this.boundNestedPath, ...names);
    }
    /**
     ...
     */
    async writeAllTo(contentMap) {
        const promises = Object.entries(contentMap).map(([to, content]) => this.writeTo(to, content));
     ...
     */
    async copyAsset(assetPath, to = assetPath) {
        const targetPath = this.outputPathTo(to);
        await this.assertSafeToWrite(targetPath);
        const dirname = path.dirname(targetPath);
        await fs.mkdir(dirname, { recursive: true });
        await fs.cp(path.join(this.options.assetsDirectory, assetPath), targetPath, {
            recursive: true,
            errorOnExist: false,
        });
        await this.markAsWritten(targetPath);
     ...
     * @protected
     */
    async generating(sources, outputs, cb) {
    ...
    async assertSafeToWrite(fullPath) {
        if (this.options.forceGenerate) {
            return;
        }
