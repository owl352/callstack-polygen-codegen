...
export function buildHostSource(generatedModules) {
    ...
    const moduleChecksums = generatedModules
        .map((module) => `{ "${module.name}", "${module.checksum.toString('hex')}" }`)
        .join(',\n      ')
        .trimEnd()
        .replace(/,$/, '');
    const moduleFactoriesMap = generatedModules
        .map((module) => `{ "${module.checksum.toString('hex')}", ${module.turboModule.moduleFactoryFunctionName} }`)
        .join(',\n      ')
        .trimEnd();
    function makeModuleFactoryDecl(module) {
        return `std::shared_ptr<Module> ${module.turboModule.moduleFactoryFunctionName}();`;
    }
    function makeModuleHandler(module) {
        return stripIndent(`if (name == "${module.name}") { return ${module.turboModule.moduleFactoryFunctionName}(); }`);
    }
    return (HEADER +
        stripIndent(`
    #include <sstream>
